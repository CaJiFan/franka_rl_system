"""
vision_module_realsense.py
==========================
Real-world vision front-end for the Tilted Wipe task on Franka Research 3.
Camera backend: Intel RealSense D4xx / L515 (via pyrealsense2).
...
Usage
-----
  python vision_module_realsense.py
...
--------------------
  ...
  s – save calibration to  calibration_realsense.npz
  l – load calibration from calibration_realsense.npz
  q – quit
...
CALIB_FILE = os.path.join(os.path.dirname(__file__), "calibration_realsense.npz")
...
# RealSense stream resolution / framerate preferences (tried in order)
RS_COLOR_PREFS = [
    (1280, 720, 30),
    (960,  540, 30),
    (640,  480, 30),
...
# ===========================================================================
# RealSense camera init helper
# ===========================================================================

def _init_realsense():
    """
    Start a RealSense pipeline with the best available color resolution.
    Returns (pipeline, profile) or raises RuntimeError.
    """
    ctx     = rs.context()
    ...
    print(f"[CAM] Found RealSense device: {dev_name}")

    pipeline = rs.pipeline()
    config   = rs.config()
    selected = None

    for w, h, fps in RS_COLOR_PREFS:
        try:
            cfg_test = rs.config()
            cfg_test.enable_stream(rs.stream.color, w, h, rs.format.bgr8, fps)
            tmp = rs.pipeline()
            tmp.start(cfg_test)
            tmp.stop()
            selected = (w, h, fps)
    ...
    if selected is None:
        # Fall back: let the SDK choose resolution
        config.enable_stream(rs.stream.color, rs.format.bgr8, 30)
        print("[CAM] Using default RealSense color stream")
    else:
        w, h, fps = selected
        config.enable_stream(rs.stream.color, w, h, rs.format.bgr8, fps)
        print(f"[CAM] RealSense color stream: {w}x{h} @ {fps} fps")
...
def run_vision_loop():
    # ── Camera init ──────────────────────────────────────────────────────────
    try:
        rs_pipeline, rs_profile = _init_realsense()
    except Exception as e:
        print(f"[VIS] Camera init failed: {e}")
        return

    # Optional: keep auto-exposure on for varying lighting conditions
    color_sensor = rs_profile.get_device().first_color_sensor()
    color_sensor.set_option(rs.option.enable_auto_exposure, 1)

    calibrator = BoardCalibrator()
    calibrator.load()

    win = "Wipe Vision Module - RealSense"
    cv2.namedWindow(win)
    ...
    try:
        while True:
            ...
            # pyrealsense2 bgr8 format → numpy array is already BGR
