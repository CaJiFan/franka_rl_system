...
CALIB_FILE = os.path.join(os.path.dirname(__file__), "calibration.npz")

# Default segmentation params (tunable via trackbars)
DEFAULT_OTSU_OFFSET = 0    # signed offset added to Otsu threshold (-50 → +50)
DEFAULT_MIN_AREA    = 50   # minimum contour area in pixels² to keep as ink
...
def segment_ink(bgr_img, board_mask, otsu_offset=0, min_area=50, close_rad=5):
    """
    ...
    if board_mask is None:
        ...
        L   = lab[:, :, 0]
        ret, raw = cv2.threshold(L, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)
...
def compute_ink_obs(mask_01, H_board, task_start_area_px, prev_target_px=None, lock_dist_px=TARGET_LOCK_DIST_PX):
    """
    ...
    """
    # Find all separate strokes
    cnts, _ = cv2.findContours((mask_01 * 255).astype(np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    ...
    # Proportion wiped: reduction in total ink since 'Start Task'
    ...
    if valid_contours:
        if prev_target_px is not None:
            # Find all contours within the lock distance of the previous target
            candidates = []
            for info in valid_contours:
                dist = np.sqrt((info["cx"] - prev_target_px[0])**2 + (info["cy"] - prev_target_px[1])**2)
                if dist < lock_dist_px:
                    candidates.append(info)
            
            if candidates:
                # Target lock successful: pick the largest contour among the nearby candidates
                target_info = max(candidates, key=lambda x: x["area"])
            else:
                # Target lost (wiped away completely or moved too fast): fall back to global largest
                target_info = max(valid_contours, key=lambda x: x["area"])
        else:
            # No previous target: pick the global largest
            target_info = max(valid_contours, key=lambda x: x["area"])

    # If no ink left or target is invalid
    ...
    # Compute properties for the LOCKED target stroke
    M = target_info["M"]
    cx_px, cy_px = target_info["cx"], target_info["cy"]
...
def build_obs_vector(ink_obs, eef_pos=None):
    ...
    if eef_pos is not None:
        g2c = centroid - np.asarray(eef_pos)
    else:
        g2c = np.zeros(3)
...
def run_vision_loop():
    sdk_pipeline = Pipeline()
    sdk_config   = Config()

    try:
        profiles      = sdk_pipeline.get_stream_profile_list(OBSensorType.COLOR_SENSOR)
        color_profile = None
        for cw, ch in [(1280, 720), (640, 480)]:
            try:
                color_profile = profiles.get_video_stream_profile(cw, ch, OBFormat.RGB, 30)
                break
            except Exception:
                continue
        if color_profile is None:
            for i in range(profiles.count()):
                p = profiles.get_video_stream_profile(i)
                if p.get_format() == OBFormat.RGB:
                    color_profile = p
                    break
        if color_profile is None:
            raise RuntimeError("No RGB profile found.")
        sdk_config.enable_stream(color_profile)
        sdk_pipeline.start(sdk_config)
        print(f"[VIS] Camera started – {color_profile.get_width()}×{color_profile.get_height()}")
    except Exception as e:
        print(f"[VIS] Camera init failed: {e}")
        return

    calibrator = BoardCalibrator()
    calibrator.load()

    win = "Wipe Vision Module"
    cv2.namedWindow(win)
    cv2.setMouseCallback(win, calibrator.on_mouse)

    cv2.createTrackbar("Otsu Offset  (-50→+50)", win, 50 + DEFAULT_OTSU_OFFSET, 100, nothing)
    cv2.createTrackbar("Min Area (px²)",          win, DEFAULT_MIN_AREA,         300, nothing)
    ...
    try:
        while True:
            frames = sdk_pipeline.wait_for_frames(100)
            if not frames:
                empty_n += 1
                if empty_n > 50:
                    break
            ...
            color_frame = frames.as_frame_set().get_color_frame()
            ...
            min_area    = max(1, cv2.getTrackbarPos("Min Area (px²)", win))
            ...
            hud = [
                ...
                f"Task Start Area: {task_start_area_px:.0f} px  ({'SET ✓' if task_started else 'NOT SET – press T'})",
