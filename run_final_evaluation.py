...
Output layout, one directory per run:
    <out-root>/<policy>/<referent>_<manner>/run<NNN>_<timestamp>/
        ...
        rollout_summary.json           # policy/referent/manner/steps/paths/counts/sync anchor

Usage (after serve_policy.py is up on the GPU machine and SSH-tunnelled, and
the crisp_controllers stack is launched per bookish-dollop-icra27/
...
--policy b1 --b1-oracle-stiffness (defaults to bookish-dollop-icra27/reports/
...
older franka_ros2_teleop/run_pilot_rollout.py path does. Documented here
...
# bookish-dollop-icra27/scripts is a src/ sibling of this package -- same
...
def run_one_recorded_episode(
    ...
    the server, and an optional b1_stiffness override (see module docstring)
    ...
    try:
        ...
        for i in range(args.n_rollouts):
            ...
            print(
                ...
                f"{steps_executed} steps executed, recorded to {run_dir}"
            )
