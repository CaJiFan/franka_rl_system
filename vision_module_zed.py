...
--------------------
  ...
  s - save calibration to  calibration_zed.npz
  l - load calibration from calibration_zed.npz
  q - quit
...
- The ZED SDK must be installed, and `pyzed` available in the python environment.
"""
...
CALIB_FILE = os.path.join(os.path.dirname(__file__), "calibration_zed.npz")
...
def run_vision_loop():
    ...
    if err != sl.ERROR_CODE.SUCCESS:
        print(f"[VIS] ZED Camera init failed: {repr(err)}")
        return
    
    print("[CAM] ZED camera opened successfully.")

    calibrator = BoardCalibrator()
    calibrator.load()

    win = "Wipe Vision Module - ZED 2i"
    cv2.namedWindow(win)
